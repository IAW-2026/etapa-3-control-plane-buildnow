...
import { X, CreditCard, Loader2, Calendar, Mail } from 'lucide-react';
import { PaymentDetailResponse } from '../../types';
import { getPaymentDetailAction } from '../actions/paymentControlPlaneActions';
    ...
    const fetchDetail = async () => {
      ...
      if (isMounted) {
        if (result.success && result.data) {
          setData(result.data as PaymentDetailResponse);
        } else {
    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col bg-[var(--color-surface)] shadow-2xl transition-transform duration-300 transform translate-x-0 overflow-y-auto border-l border-[var(--color-outline-variant)]">
        ...
        <div className="flex items-center justify-between border-b border-[var(--color-outline-variant)] px-6 py-4">
          <h2 className="text-lg font-semibold text-[var(--color-on-surface)]">Detalles de Pago</h2>
          <button
            onClick={onClose}
          ...
          {!loading && !error && data && (
            <>
              ...
              <div className="flex flex-col items-center justify-center text-center p-6 rounded-xl border border-[var(--color-outline-variant)] bg-[var(--color-surface-container-lowest)]">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-100 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400 mb-3">
                  <CreditCard className="h-6 w-6" />
                </div>
                <p className="text-3xl font-extrabold text-[var(--color-on-surface)]">
                  ${Number(data.amount).toLocaleString('es-AR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
                <div className="mt-2">
              ...
              <section className="space-y-3">
                ...
                <div className="rounded-xl border border-[var(--color-outline-variant)] p-4 space-y-3 bg-[var(--color-surface-container-lowest)] text-sm">
                  <div className="flex justify-between">
                    <span className="text-[var(--color-on-surface-variant)]">Mercado Pago ID</span>
                    <span className="font-medium text-[var(--color-on-surface)]">{data.mercadopagoId || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-[var(--color-on-surface-variant)]">Preference ID</span>
                    <span className="font-medium text-[var(--color-on-surface)] truncate max-w-[180px]" title={data.preferenceId || ''}>{data.preferenceId || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className="text-[var(--color-on-surface-variant)]">User ID</span>
                    <span className="font-medium text-[var(--color-on-surface)] truncate max-w-[180px]">{data.userId}</span>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="rounded-xl border border-[var(--color-outline-variant)] p-4 space-y-3 bg-[var(--color-surface-container-lowest)] text-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-[var(--color-on-surface-variant)] flex items-center gap-1"><Mail className="h-3.5 w-3.5" /> Email</span>
                    <span className="font-medium text-[var(--color-on-surface)]">{data.payerEmail || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[var(--color-on-surface-variant)]">Método de Pago</span>
                    <span className="font-medium text-[var(--color-on-surface)] capitalize">{data.method || 'N/A'}</span>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-3">
                ...
                <div className="rounded-xl border border-[var(--color-outline-variant)] p-4 space-y-3 bg-[var(--color-surface-container-lowest)] text-sm">
                  <div className="flex justify-between items-center">
                    <span className="text-[var(--color-on-surface-variant)] flex items-center gap-1"><Calendar className="h-3.5 w-3.5" /> Creado</span>
                    <span className="font-medium text-[var(--color-on-surface)]">
                      {new Date(data.createdAt).toLocaleString('es-AR')}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-[var(--color-on-surface-variant)] flex items-center gap-1"><Calendar className="h-3.5 w-3.5" /> Pagado</span>
                    <span className="font-medium text-[var(--color-on-surface)]">
                      {data.paidAt ? new Date(data.paidAt).toLocaleString('es-AR') : 'Pendiente'}
                    </span>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-3">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-[var(--color-on-surface-variant)]">
                  Transacciones Relacionadas ({data.transactions?.length || 0})
                </h3>
                {data.transactions && data.transactions.length > 0 ? (
                  ...
                  <p className="text-sm text-[var(--color-on-surface-variant)]">No hay transacciones registradas.</p>
                )}
              </section>
            </>
          )}
