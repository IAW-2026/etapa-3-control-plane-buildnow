...
import { TransactionListResponse, TransactionStatus, TransactionType } from '../../types';
    ...
    <div className="rounded-xl border border-[var(--color-outline-variant)] bg-[var(--color-surface)] shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 border-b border-[var(--color-outline-variant)]">
        <form onSubmit={handleSearch} className="relative w-full sm:max-w-xs">
          ...
          <input
            type="text"
            placeholder="Buscar transacciones (Order ID)..."
            value={searchValue}
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          >
            ...
            <option value="PAYOUT_DELIVERY">Payout Delivery</option>
            <option value="PAYOUT_SELLER">Payout Seller</option>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-[var(--color-outline-variant)] text-sm text-left">
          <thead className="bg-[var(--color-surface-container-low)]">
            <tr>
              <th className="px-6 py-3 font-medium text-[var(--color-on-surface-variant)]">Transaccion ID</th>
              <th className="px-6 py-3 font-medium text-[var(--color-on-surface-variant)]">Tipo</th>
              ...
              <th className="px-6 py-3 font-medium text-[var(--color-on-surface-variant)]">Entidad Relacionada(ID)</th>
              <th className="px-6 py-3 font-medium text-[var(--color-on-surface-variant)]">Fecha</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--color-outline-variant)]">
            {initialData.items && initialData.items.length > 0 ? (
              initialData.items.map((tx) => (
                <tr key={tx.id} className="hover:bg-[var(--color-surface-container-lowest)] transition-colors">
                  <td className="px-6 py-4 font-mono text-xs text-[var(--color-on-surface)] max-w-[120px] truncate" title={tx.id}>
                    {tx.id}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <GeneralStatusBadge status={tx.status} />
                  </td>
                  <td className="px-6 py-4 font-mono text-xs text-[var(--color-on-surface-variant)] max-w-[150px] truncate">
                    {tx.paymentId ? `Payment: ${tx.paymentId}` : tx.payoutId ? `Payout: ${tx.payoutId}` : 'N/A'}
                  </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-[var(--color-outline-variant)] px-6 py-4">
          <p className="text-sm text-[var(--color-on-surface-variant)]">
            ...
            de <span className="font-medium text-[var(--color-on-surface)]">{initialData.total}</span> transacciones
          </p>
